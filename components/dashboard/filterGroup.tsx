      ...
      <div className="relative">
        <button
          ...
          className={cn(
            ...
            isOpen && "ring-2 ring-[#6691dc]/15 border-[#6691dc]/30",
            isPending && "opacity-70" 
          )}
        >
          <span className="truncate mr-1 text-left">{getLabel(id, label)}</span>
          <div className="flex items-center gap-1">
            {isPending && openDropdown === id && <Loader2 className="h-2.5 w-2.5 animate-spin text-[#6691dc]" />}
            ...
            {id !== "year" && (
              <>
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-2.5 text-[11px] hover:bg-[#6691dc]/10 text-slate-700 font-bold border-b border-slate-50 cursor-pointer"
                >
                  All {label}s
                  {!searchParams.get(id) && <Check className="h-3.5 w-3.5 text-[#6691dc] stroke-[3px]" />}
                </button>
              </>
            )}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "bg-[#6691dc]/15 text-[#6691dc] font-bold" : "hover:bg-slate-50 text-slate-600"
                  )}
                >
                  <span className="truncate pr-2">{display}</span>
                  {active && <Check className="h-3.5 w-3.5 text-[#6691dc] stroke-[3px]" />}
                </button>
              );
            })}
          </div>
